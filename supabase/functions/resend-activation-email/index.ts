...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const render = (href: string) =>
      p(`${greeting}`) +
      p(`Here is a fresh link to activate your iTrova account for <strong style="color:${BRAND.deep};">${esc(biz.name ?? "your business")}</strong>.`) +
      `      <div style="margin:18px 0 0;">${emailButton(href, "Activate my account")}</div>` +
      small(`Or copy this address into your browser:<br><a href="${esc(href)}" style="color:#0d6b52;word-break:break-all;">${esc(href)}</a>`) +
      small("The link signs you in and marks your email as activated. It expires after a short while, so use it soon.") +
      small(esc(closing)) +
      `      <div style="margin-top:22px;">` + p("The iTrova team") + `</div>`;
    const document = (href: string) => emailShell({
      title: SUBJECT,
      preheader: `A fresh activation link for ${biz.name ?? "your iTrova account"}.`,
      body: render(href),
    });
    const html = document(actionLink);
    ...
    try {
      const res = await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({ from: `${fromName} <${fromEmail}>`, to: [toEmail], subject: SUBJECT, html, text: toPlainText(html), ...(replyTo ? { reply_to: replyTo } : {}) }),
